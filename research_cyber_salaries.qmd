...
Jeder Datensatz enthält den Titel des jeweiligen Berufes der Person. Im folgenden Abschnitt werden diese untersucht auf Häufigkeit sowie auf deren durchschnittliche Gehälter. Jedoch ist anzumerken, dass nur wenig über die einzelnen Berufstitel ausgesagt werden kann, da die genaue Tätigkeit vom jeweiligen Unternehmen abhängt und unterschiedliche Tätigkeiten auch in einem Berufstitel zusammengefasst werden können.


### 4.2.1 Häufigkeit der Berufstitel 
Bei den einzelnen Titeln der Berufe ist es schwer herauszufinden was genau die Bedeutung der Position der einzelenen Person ist, da diese gerne durch das Unternehmen selbst definiert werden. Aufgrunddessen werden in der folgenden Darstellung die einzelen Titel nach Häufigkeit gruppiert und die einzelnen Gruppen in Prozent angegeben. Es werden nur die `r paste(count)` häufigsten Titel angegeben, alle anderen werden in der Gruppe `Andere` zusammengefasst.
...
```
Zu sehen ist erstmal das ein großer Teil in der Gruppe `Andere` zusammengefasst wurde. Dies könnte damit zusammenliegen, dass es entweder eine große Anzahl an unterschiedlichen Berufen innerhalb des IT-Security Sektors existieren oder gleiche Berufe in den Betrieben unterschiedlich benannt werden. Jedoch ist davon auszugehen, dass es sich um eine starke Verteilung handelt, da kein Berufstitel in den `43%` der Gruppe `Andere` die `2%` Hürde überschreitet.
Trotzdessen ist zu erkennen das ein größerer Trend Trend in den Berufen ``r paste(titles[1])`` und ``r paste(titles[2])`` besteht.  

#### 4.2.2 Durchschnittliche Gehälter pro Berufstitel
Nachfolgend werden noch die durchschnittlichen Gehälter pro Berufstitel dargestellt mit der jeweiligen Verteilung. Hierbei werden nur die `r paste(count)` häufigsten Berufstitel berücksichtigt, die oben in der Verteilung der Berufstitel bereits dargestellt wurden.
...
```
Wie zu vermuten sind Berufe mit enhaltenen Titlen: Manager und Officer im Gehalt etwas höher als die anderen. Jedoch gibt es zum einem den Außreiser ``r paste(titles[1])``, der das höchste durchschnittlichste Gehalt ausmacht und zum anderen gibt es auch den Außreiser ``r paste(titles[10])`` der womöglich ein spezieler Beruf ist und somit auch ein höheres Gehalt hat.

Die Verteilung der Gehälter ist bei den meisten Berufstiteln recht gleichmäßig, jedoch gibt es auch hier Ausreißer. Dies könnte damit zusammenhängen, dass es sich bei den Gehältern um die Gehälter der jeweiligen Länder handelt.
Aufgrunddessen wird in der folgenden Darstellung die einzlenen Länder mit der jeweiligen durchschnittlichen Bezahlung dargestellt. Als Refeferenz Titel wurde hierbei ``r paste(titles[9])`` genommen, da dieser eine starke Verteilung ausmacht.
```{r}
#| code-fold: true
#| message: false
#| warning: false
...
# calculate the mean salary of the job "information security officer" for each country
...
# plot the job_title_mean_salary in a bar plot with a bar for each country
...
```
Der Vermutung fürsprechend ist zu erkennen, dass das Gehalt in den ``USA`` höher als in den restlichen Länderen. Auch sind deutlich mehr Tätige in den ``USA`` vorhanden als in den anderen Länden die in der Darstellung zu erkennen sind.
...
ggplot(everage_salary_per_country, aes(x = reorder(company_location, mean_salary), y = mean_salary)) +
    ...
    coord_flip()
```
Die ausgewerteten Gehälter der einzelnen Länder sind auch mit den ungefähren Durchschnittsgehältern der Länder vergleichbar. Jedoch wird auf eine tiefere Analyse mit der durchschnittlichen Bezahlung der einzelnen Ländern verzichtit, da dies nicht der Fokus der Auswertung ist.
...
Auch bei Senior Angestellten ist der Unterschied deutlich größer als bei den Junior Angestellten. Mit rund `r round(company_size_stats_exp[3, "mean"])` USD, `r round(company_size_stats_exp[7, "mean"])` USD und `r round(company_size_stats_exp[11, "mean"])` USD verdienen Senior Angestellte in mittelgroßen Betrieben am besten. Interssant ist, dass Senior Angestellen von kleinen Unternehmen ähnlich viel verdienen, wie Intermediate Angestellte von großen bzw. mittelgroßen Unternehmen.

Zuletzt verdenienen Executive Angestellte am Meisten. Hier ist es aber schwierig eine allgemeine Aussage aus dem Datensatz zu ziehen, weil nur 5 Datensätze von Executive Mitarbeiter aus kleinen Unternehmen und 16 aus mittelgroßen Unternehmen vorliegen. Dennoch kann gesagt werden, dass sie am meisten im Schnitt verdienen.

# 4.6 Vertragsart

Im zu grunde liegenden Datensatz gibt es verschiedene Vertragsarten. Im Folgenden sollen diese Vertragsarten genauer betrachtet werden. Hierfür wird zuerst betrachtet, welche Arten zu welchem Anteil enthalten sind.
...
Im folgenden Abschnitt werden nun noch die Gehälter der Angestellten in Abhängigkeit davon, ob zu wie viel Prozent sie remote arbeiten (0%, 50%, 100%), betrachtet. Hierzu wird zusammengefasst die einzelnen Remoteraten mit der Anzahl der Angestellten und dem durchschnittlichen Gehalt dargestellt.
...
ggplot(remote_ratio_salary_and_count, aes(x = remote_ratio, y = mean, fill = remote_ratio)) +
    ...
    geom_text(aes(label = paste(round(mean), "$", sep = "")), vjust = 2, size = 5) +
    theme_minimal() +
    scale_fill_manual(values=wes_palette(n = 3, name = color_palette)) +
    theme(
        legend.position = "top",
...
Zu erkennen ist, dass die meisten Angestellten zu 100% remote arbeiten. Auch interessant ist, dass die Angestellten, die zu 100% remote arbeiten im durchschnitt ein höhreres Gehalt erhalten als Angestellte die 0% oder 50% remote arbeiten.
